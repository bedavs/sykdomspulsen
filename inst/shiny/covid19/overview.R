...
overview_server <- function(input, output, session, config) {
  ...
  output$overview_plot_national_syndromes_proportion <- renderCachedPlot({
    ...
    pd[, `Influensa (R80)` := consult_with_influenza - consult_without_influenza]
    setnames(pd,c("n"),c("COVID-19 (R99.1)"))

    pd <- pd[,c("date","COVID-19 (R99.1)","Influensa (R80)","consult_with_influenza"),with=F]
    pd <- melt.data.table(pd, id.vars = c("date","consult_with_influenza"))

    q <- ggplot(pd, aes(x=date,y=100*value/consult_with_influenza, color=variable, group=variable))
  ...
  output$overview_plot_county_proportion <- renderCachedPlot({
    ...
    pd[,n_per_100_consultation:=100*n/consult_with_influenza]

    labels <- pd[date == max(date)]
    labels[,lab := format_nor_perc(n_per_100_consultation)]

    max_val <- max(pd$n_per_100_consultation)

    q <- ggplot(pd, aes(x=date,y=n_per_100_consultation))
    ...
    q <- q + theme(legend.key.size = unit(1, "cm"))
    q <- q + labs(title="Andel konsultasjoner som tilhører COVID-19 (R99.1)")
